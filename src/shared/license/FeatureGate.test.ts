...
import { FeatureGate, FeatureGateError, FeatureGateErrorCode } from './FeatureGate'
...
jest.mock('../../main/database/DatabaseService')
...
describe('FeatureGate - Static Utility Methods', () => {
  ...
  beforeEach(() => {
    ...
    mockLicenseService = new MockedLicenseService(mockDatabaseService) as jest.Mocked<LicenseService>

    // Setup default mock responses
    mockLicenseService.initialize = jest.fn()
    mockLicenseService.checkFeatureAvailability = jest.fn()
    mockLicenseService.requireFeature = jest.fn()
    mockLicenseService.getLicenseInfo = jest.fn()
  })
  ...
  describe('TC-014: FeatureGate.isAvailable() Method', () => {
    it('should return true for available features', async () => {
      ...
      mockLicenseService.checkFeatureAvailability.mockResolvedValue(featureAvailability)
      ...
      expect(mockLicenseService.checkFeatureAvailability).toHaveBeenCalledWith('basic_notes')
    })
    ...
    it('should return false for unavailable features', async () => {
      ...
      // Act
      const isAvailable = await FeatureGate.isAvailable('cloud_sync', mockLicenseService)

      // Assert
      expect(isAvailable).toBe(false)
      expect(mockLicenseService.checkFeatureAvailability).toHaveBeenCalledWith('cloud_sync')
    })

    it('should handle license service errors gracefully', async () => {
      // Arrange
      mockLicenseService.checkFeatureAvailability.mockRejectedValue(new Error('License service error'))

      // Act & Assert
      await expect(FeatureGate.isAvailable('basic_notes', mockLicenseService))
        .rejects.toThrow(FeatureGateError)
    })

    it('should validate feature name parameter', async () => {
      // Act & Assert
      await expect(FeatureGate.isAvailable('', mockLicenseService))
        .rejects.toThrow(new FeatureGateError(FeatureGateErrorCode.INVALID_FEATURE_NAME, 'Feature name cannot be empty'))
      
      await expect(FeatureGate.isAvailable(null as any, mockLicenseService))
        .rejects.toThrow(FeatureGateError)
      
      await expect(FeatureGate.isAvailable(undefined as any, mockLicenseService))
        .rejects.toThrow(FeatureGateError)
    })

    it('should validate license service parameter', async () => {
      // Act & Assert
      await expect(FeatureGate.isAvailable('basic_notes', null as any))
        .rejects.toThrow(new FeatureGateError(FeatureGateErrorCode.INVALID_LICENSE_SERVICE, 'License service is required'))
      
      await expect(FeatureGate.isAvailable('basic_notes', undefined as any))
        .rejects.toThrow(FeatureGateError)
    })
  })
  ...
  describe('TC-014: FeatureGate.getAvailability() Method', () => {
    it('should return complete FeatureAvailability object', async () => {
      ...
      const expectedAvailability: FeatureAvailability = {
        ...
        available: false,
        reason: 'license_required',
        upgradeMessage: 'Upgrade to Standard plan for advanced formatting features'
      }
      mockLicenseService.checkFeatureAvailability.mockResolvedValue(expectedAvailability)

      // Act
      const availability = await FeatureGate.getAvailability('advanced_formatting', mockLicenseService)

      // Assert
      expect(availability).toEqual(expectedAvailability)
      expect(mockLicenseService.checkFeatureAvailability).toHaveBeenCalledWith('advanced_formatting')
    })
    ...
    it('should return availability for free tier features', async () => {
      ...
      // Act
      const availability = await FeatureGate.getAvailability('basic_notes', mockLicenseService)
    ...
    it('should handle different unavailability reasons', async () => {
      ...
      for (const testCase of testCases) {
        ...
        // Act
        const availability = await FeatureGate.getAvailability(testCase.feature, mockLicenseService)
  ...
  describe('TC-014: FeatureGate.requireFeature() Method', () => {
    it('should not throw for available features', async () => {
      ...
      // Act & Assert
      await expect(FeatureGate.requireFeature('basic_notes', mockLicenseService))
        .resolves.not.toThrow()
      
      expect(mockLicenseService.requireFeature).toHaveBeenCalledWith('basic_notes')
    })
    ...
    it('should throw for unavailable features', async () => {
      ...
      expect(mockLicenseService.requireFeature).toHaveBeenCalledWith('cloud_sync')
    })

    it('should provide helpful error messages', async () => {
      // Arrange
      const licenseError = new Error('Feature cloud_sync requires paid license')
      mockLicenseService.requireFeature.mockRejectedValue(licenseError)
      ...
      try {
        ...
        expect(featureError.code).toBe(FeatureGateErrorCode.FEATURE_REQUIRED_BUT_UNAVAILABLE)
  ...
  describe('FeatureGate - Bulk Operations', () => {
    it('should check multiple features availability efficiently', async () => {
      ...
      const expectedResults = [
        ...
        { feature: 'cloud_sync', available: false, reason: 'license_required' as const },
        { feature: 'advanced_formatting', available: false, reason: 'license_required' as const }
      ]
      ...
      // Act
      const results = await FeatureGate.checkMultipleFeatures(features, mockLicenseService)

      // Assert
      expect(results).toHaveLength(3)
      expect(results).toEqual(expectedResults)
      expect(mockLicenseService.checkFeatureAvailability).toHaveBeenCalledTimes(3)
    })

    it('should filter available features from a list', async () => {
      // Arrange
      const features = ['basic_notes', 'cloud_sync', 'simple_positioning', 'advanced_formatting']
      ...
      mockLicenseService.checkFeatureAvailability
        .mockImplementation(async (feature: string) => {
          ...
          return {
            ...
            reason: freeFeatures.includes(feature) ? undefined : 'license_required' as const
          }
        })

      // Act
      const availableFeatures = await FeatureGate.getAvailableFeatures(features, mockLicenseService)
      ...
      mockLicenseService.checkFeatureAvailability
        .mockImplementation(async (feature: string) => {
          ...
          return {
            ...
            available: freeFeatures.includes(feature),
            reason: freeFeatures.includes(feature) ? undefined : 'license_required' as const,
            upgradeMessage: freeFeatures.includes(feature) ? undefined : `Upgrade to access ${feature}`
          }
        })

      // Act
      const unavailableFeatures = await FeatureGate.getUnavailableFeatures(features, mockLicenseService)
  ...
  describe('FeatureGate - Caching and Performance', () => {
    ...
    it('should respect cache TTL and refresh stale entries', async () => {
      ...
      // Assert
      expect(mockLicenseService.checkFeatureAvailability).toHaveBeenCalledTimes(2)
    })
...
describe('FeatureGate - Integration Scenarios', () => {
  ...
  beforeEach(() => {
    ...
    mockLicenseService.checkFeatureAvailability = jest.fn()
    mockLicenseService.getLicenseInfo = jest.fn()
    mockLicenseService.activateLicense = jest.fn()
  })
  ...
  describe('License Transition Scenarios', () => {
    it('should update feature availability after license activation', async () => {
      ...
      mockLicenseService.checkFeatureAvailability
        .mockImplementation(async (feature: string) => ({
          ...
          reason: ['basic_notes', 'simple_positioning'].includes(feature) ? undefined : 'license_required' as const
        }))

      // Act - check feature before license activation
      const beforeActivation = await FeatureGate.isAvailable('cloud_sync', mockLicenseService)
      ...
      mockLicenseService.activateLicense.mockResolvedValue({
        ...
        limits: {
          ...
          featuresEnabled: ['basic_notes', 'simple_positioning', 'cloud_sync', 'advanced_formatting']
        }
      })
      ...
      const afterActivation = await FeatureGate.isAvailable('cloud_sync', mockLicenseService)
    ...
    it('should handle license deactivation gracefully', async () => {
      ...
      // Act
      const beforeDeactivation = await FeatureGate.isAvailable('cloud_sync', mockLicenseService)
      
      // Simulate license deactivation
      FeatureGate.clearCacheForLicense()
      
      const afterDeactivation = await FeatureGate.isAvailable('cloud_sync', mockLicenseService)
  ...
  describe('Error Recovery Scenarios', () => {
    it('should fallback gracefully when license service is unavailable', async () => {
      // Arrange
      mockLicenseService.checkFeatureAvailability.mockRejectedValue(new Error('Service unavailable'))

      // Act & Assert
      await expect(FeatureGate.isAvailable('basic_notes', mockLicenseService, { fallbackToConservative: true }))
    ...
    it('should retry on transient license service errors', async () => {
      ...
      // Act
      const result = await FeatureGate.isAvailable('basic_notes', mockLicenseService, { retryCount: 1 })
